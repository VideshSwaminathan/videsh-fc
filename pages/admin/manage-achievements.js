...
export default function ManageAchievements() {
  ...
  async function handleDelete(id) {
    ...
    const res = await fetch('/api/achievements', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ id }) })
  ...
  async function handleSave() {
    setSaving(true)
    const res = await fetch('/api/achievements', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(editing) })
  ...
  return (
    <AdminLayout title="Manage Achievements">
      <div style={{ marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2rem', fontWeight: 600 }}>Manage Achievements</h1>
      </div>
      ...
      {loading ? <p style={{ color: '#6B6B6B' }}>Loading...</p> : items.length === 0 ? <p style={{ color: '#6B6B6B' }}>No achievements yet.</p> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map(a => (
            <div key={a.id} style={{ background: 'white', border: '1px solid #E8E0D0', borderRadius: 12, overflow: 'hidden' }}>
              {editing?.id === a.id ? (
                <div style={{ padding: '1.25rem' }}>
                  <div style={{ marginBottom: 12 }}>
                    <label style={{ fontSize: 11, color: '#6B6B6B', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Icon</label>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {EMOJIS.map(em => (
                        <button key={em} type="button" onClick={() => setField('emoji', em)} style={{ width: 38, height: 38, borderRadius: 8, border: `1px solid ${editing.emoji === em ? '#C9A84C' : '#E8E0D0'}`, background: editing.emoji === em ? 'rgba(201,168,76,0.1)' : 'transparent', cursor: 'pointer', fontSize: 18 }}>{em}</button>
                      ))}
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                    ...
                    <div>
                      <label style={{ fontSize: 11, color: '#6B6B6B', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 4 }}>Category</label>
                      <select value={editing.category || 'Award'} onChange={e => setField('category', e.target.value)} className="input-field" style={{ fontSize: 13 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                    ...
                    <span style={{ fontSize: 13 }}>⭐ Feature on home page</span>
                  </div>
                ...
                <div style={{ padding: '0.9rem 1.25rem', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: '#6B6B6B' }}>{a.category} · {format(new Date(a.date), 'MMM d, yyyy')}</div>
                  </div>
                  {a.featured && <span className="gold-badge">Featured</span>}
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => setEditing({ ...a })} style={{ background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.3)', color: '#C9A84C', borderRadius: 6, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontFamily: 'DM Sans, sans-serif', fontWeight: 500 }}>Edit</button>
                    <button onClick={() => handleDelete(a.id)} style={{ background: 'rgba(248,215,218,0.5)', border: '1px solid rgba(220,53,69,0.2)', color: '#dc3545', borderRadius: 6, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontFamily: 'DM Sans, sans-serif', fontWeight: 500 }}>Delete</button>
                  </div>
                </div>
              )}
            </div>
